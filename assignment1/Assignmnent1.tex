...
%\graphicspath{{D:/figs/Figure_1.pdf}}
%\title{
...
Measure $\angle{ADC}$ and write it down.\\
	...
	Let O be the centre of the semicircle.\\
	    ...
	    {\implies}$It's radius "r"= \dfrac{7}{2}cm= 3.5cm.$\\
    ...
    \begin{tabular}{|c|c|c|} \hline
        ...
        $r$               & 3.5               & Input          \\ \hline
        $\angle BAC=\theta$ &$90^\circ$   &Input       \\ \hline 
        $\angle ABC$   &$\dfrac{180-\theta}{2}=45^\circ$   &Calculated       \\ \hline
        $\angle DBC$   &$\dfrac{180-\theta}{4}=22.5^\circ$    &Calculated      \\ \hline
        $\vec{O}$       & $\vec{0}$        & Input  \\\hline
        $\vec{A}$       &\myvec{0\\3.5}& Input\\\hline
        $\vec{B}$       &  $\myvec{-3.5\\0}$ & Input\\\hline
        $\vec{C}$       &\myvec{3.5\\0}& Input\\\hline
        $\vec{D}$       & $\myvec{-3.5+2r\cos^2{\dfrac{180-\theta}{4}^\circ}\\2r\cos\dfrac{180-\theta}{4}\sin\dfrac{180-\theta}{4}^\circ}$ & Output\\\hline
...
\end{table}
	  ...
	  Equation \eqref{eq:8},\\
	 \begin{align}&{\implies}45{\degree}+\angle{ADC}=180{\degree}\\  &{\implies}\angle{ADC}= 135{\degree} \end{align}\\
 \therefore \text{\fbox{The measure of $\angle ADC$ is 135{\degree}}}\\
%\end{flushleft}	  
\end{document}
